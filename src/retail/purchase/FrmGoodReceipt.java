...
/*udf
 * FrmGoodReceipt.javap
 *
 * Created on Jul 15, 2010, 4:49:38 PM
 */

package retail.purchase;
...
//        tblPR.getColumn("PPN").setMinWidth(0); tblPR.getColumn("PPN").setMaxWidth(0); tblPR.getColumn("PPN").setPreferredWidth(0);
    ...
    private void udfSave(){
        ...
        try{
            String sSql="select fn_phar_get_no_goodreceipt('001') as no_gr";
            String sNoGr="";
            ...
            sSql="insert into phar_good_receipt(good_receipt_id, tanggal, no_po, no_inv_do_sj, remarks, site_id, " +
                    "flag_trx, user_ins, kode_supp, source_id) values('"+sNoGr+"', " +
                    "now(), " + //
                    "'"+txtNoPO.getText()+"', '"+txtNoSJ.getText()+"', '"+txtRemark.getText()+"', " +
                    "'"+txtSite.getText()+"', 'T', '"+MainForm.sUserName+"', '"+txtSupplier.getText()+"', '"+txtNoGR.getText()+"'); ";

            for(int i=0; i<tblItem.getRowCount(); i++){
                sSql+="insert into phar_good_receipt_detail(good_receipt_id, kode_barang, jumlah, harga, discount, " +
                        "ppn, expired_date, user_ins, uom_gr, konv, jml_kecil, urut, no_pr) values('"+sNoGr+"'," +
                        ...
                        (tblItem.getValueAt(i, col.getColumnIndex("Expired")).toString().length()==0? "'1900-01-01'" : "'"+new SimpleDateFormat("dd/MM/yy").parse(tblItem.getValueAt(i, col.getColumnIndex("Expired")).toString())+"'")+"," +
...
//    }
    ...
    private void initComponents() {
        ...
        txtNoSJ.setFont(new java.awt.Font("Dialog", 0, 12));
        ...
        jPanel2.add(jLabel31, new org.netbeans.lib.awtextra.AbsoluteConstraints(140, 50, 90, 20));
        ...
        jPanel2.add(txtNetto, new org.netbeans.lib.awtextra.AbsoluteConstraints(230, 50, 120, 20));
        ...
        setBounds((screenSize.width-776)/2, (screenSize.height-558)/2, 776, 558);
    ...
    private void txtSupplierKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtSupplierKeyReleased
        fn.lookup(evt, new Object[]{lblSupplier}, "select kode_supp, coalesce(nama_supp,'') as nama_supp, coalesce(telepon,'') as telepon, " +
                "coalesce(t.jatuh_tempo, 0) as top " +
                "from r_supplier  s " +
                "left join m_termin t on t.kode=s.termin " +
                "where kode_supp||coalesce(nama_supp,'')||coalesce(s.telepon,'') ilike '%"+txtSupplier.getText()+"%' order by 2",
...
//                            return;
                ...
                case KeyEvent.VK_INSERT:{
                    ...
                    if(txtNoPO.getText().trim().length()>0){
                        ...
                        sFilter="(kode_item||nama_item)";
                        d1.setTitle("Lookup Item from PO");
                    }else{
                        sQry="select * from(" +
                             "select kode_item, coalesce(nama_item,'') as nama_item from r_item order by 2) x " ;
                        sFilter="(kode_item||nama_item)";
                        d1.setTitle("Lookup Master Item");
                    ...
                    d1.udfLoad(conn, sQry, sFilter, null);
