    ...
    private void udfLoadAP(){
        try{
            ...
            ResultSet rs=conn.createStatement().executeQuery(
                    "select * from fn_r_show_ap('"+txtSupplier.getText()+"') as (no_receipt varchar, tanggal date, jt_tempo date, " +
...
//            }
    ...
    private void txtSupplierKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtSupplierKeyReleased
        fn.lookup(evt, new Object[]{txtNamaSupplier}, "select kode_supp, coalesce(nama_supp,'') as nama_supplier from r_supplier " +
                "where kode_supp||coalesce(nama_supp,'') ilike '%"+txtSupplier.getText()+"%' order by 2", txtSupplier.getWidth()+txtNamaSupplier.getWidth()+18, 200);
