    ...
    private void udfLoadAR(){
        try{
            ...
            ResultSet rs=conn.createStatement().executeQuery(
                    "select * from fn_ar_load_outstanding_cust("+txtCustomer.getText()+") as "
                    + "(id bigint, keterangan varchar, invoice varchar, invoice_date date, "
                    + "invoice_amount double precision, paid_amount double precision, owing double precision)");
            while(rs.next()){
...
//            }
    ...
    private void udfSave(){
        ...
        try{
            ...
            ResultSet rs=conn.createStatement().executeQuery("select fn_r_get_ar_no('"+ymd.format(dmy.parse(jFTanggal.getText()))+"')");
            if(rs.next())
            ...
            TableColumnModel col=jTable1.getColumnModel();
            String sQry="INSERT INTO ar_receipt(ar_no, tanggal, id_customer, pay_for, alat_bayar, rate, " +
                        "memo, date_ins, " +
                        "user_ins)    VALUES (" +
                        "'"+txtNoTrx.getText()+"', '"+ymd.format(dmy.parse(jFTanggal.getText()))+"', "
                        + "'"+txtCustomer.getText()+"', '', '1', 1, " +
                        "'"+txtKeterangan.getText()+"', now()," +
                        "'"+MainForm.sUserName+"');\n";
            
            double bayar=0;
            for(int i=0; i<jTable1.getRowCount(); i++){
                bayar=fn.udfGetDouble(jTable1.getValueAt(i, col.getColumnIndex("Bayar")));
                if(bayar>0){
                    sQry+="INSERT INTO ar_receipt_detail(ar_no, id_penjualan, " +
                            "bayar, discount, " +
                            "date_ins) VALUES (" +
                            "'"+txtNoTrx.getText()+"', "
                            + ""+fn.udfGetDouble(jTable1.getValueAt(i, col.getColumnIndex("IDPenjualan")))+", " +
                            bayar+", "+fn.udfGetDouble(jTable1.getValueAt(i, col.getColumnIndex("Diskon")))+", " +
                            "now());\n";
                }
            }
    ...
    private void initComponents() {
        ...
        jPanel1.add(txtKeterangan, new org.netbeans.lib.awtextra.AbsoluteConstraints(100, 60, 600, 20));

        getContentPane().add(jPanel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 60, 720, 90));
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "No. Penjualan", "Tanggal", "Nilai Penjualan", "Terbayar", "Sisa", "Bayar", "Diskon", "IDPenjualan"
            }
        ) {
        ...
        getContentPane().add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 150, 720, 160));
    ...
    private void txtCustomerKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtCustomerKeyReleased
        fn.lookup(evt, new Object[]{txtNamaCustomer}, 
                "select id_customer::varchar as id, coalesce(nama_customer,'') as nama_customer "
                + "from m_customer " +
                "where id_customer::Varchar||coalesce(nama_customer,'') ilike '%"+txtCustomer.getText()+"%'", 
                txtCustomer.getWidth()+txtNamaCustomer.getWidth()+20, 200);
