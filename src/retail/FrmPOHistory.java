 ...
 * @author ustadho
 */
public class FrmPOHistory extends javax.swing.JInternalFrame {
    ...
    public FrmPOHistory(Connection con) {
        initComponents();
        this.conn=con;
...
//        tblDetail.getColumn("SisaPR").setMinWidth(0); tblDetail.getColumn("SisaPR").setMaxWidth(0); tblDetail.getColumn("SisaPR").setPreferredWidth(0);
    ...
    private void udfLoadPODetail(){
        ...
        String s ="select x.*, gabung(coalesce(y.no_gr,'')) as gr_id, " +
                "gabung(to_char(y.tanggal, 'dd/MM/yy')||' '||to_char(y.date_ins, 'hh24:MI')) as tgl_gr, sum(coalesce(y.qty_gr,0)) as qty_gr, " +
                "x.qty-sum(coalesce(y.qty_gr,0)) as sisa_po " +
                "from (" +
                "       select po.no_po, po.kode_supp, coalesce(s.nama_supp,'') as nama_supp, to_char(po.tanggal, 'dd/MM/yyyy') as tanggal, " +
                ...
                "       d.kode_item, coalesce(i.nama_item,'') as nama_item, qty, d.unit " +
                "       from r_po po" +
                ...
                "       where po.no_po='"+sNoPo+"' " +
                ") x " +
                "left join (select gr.no_gr, gr.tanggal, gr.date_ins,  gr.no_po, grd.kode_item, " +
                "       sum(coalesce(grd.qty,0)) as qty_gr " +
                "       from r_gr gr " +
                "       left join r_gr_detail grd on grd.no_gr=gr.no_gr" +
                "       where gr.no_po='"+sNoPo+"' " +
                "       group by gr.no_gr, gr.tanggal, gr.date_ins,  gr.no_po, grd.kode_item " +
                ")y on y.kode_item=x.kode_item " +
                "group by x.no_po, x.kode_supp, x.nama_supp, x.tanggal, x.jam, x.catatan, x.top, x.alamat, x.telp_supp, " +
                "x.kode_item, x.nama_item, x.qty, x.unit";
    ...
    private void udfInitForm(){
        ...
        try {
            ...
            Logger.getLogger(FrmPOHistory.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void udfLoadPO(){
        try{
            ...
            String sQry="select distinct po.no_po, po.kode_supp, coalesce(s.nama_supp,'') as nama_supp, po.tanggal, " +
                    "po.date_ins , coalesce(po.catatan,'') as remark," +
                    "coalesce(po.top,0) as top, coalesce(s.alamat_1,'')  as alamat, coalesce(s.telepon,'') as telp_supp " +
                    "from r_po po " +
                    "inner join r_po_detail d on d.no_po=po.no_po " +
                    "left join r_supplier s on s.kode_supp=po.kode_supp " +
                    ...
                    "and coalesce(s.nama_supp,'')||po.kode_supp||po.no_po ilike '%"+txtSupplier.getText()+"%' " +
...
//                }
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Purchase Order Hitory");
        ...
        jLabel16.setText(" Histori Pesanan Pembelian");
        ...
        tblDetail.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "No.", "Product ID", "Keterangan", "Satuan", "Qty", "Good Receipt#", "Receipt. Time", "Qty Receipt"
            }
        ) {
            Class[] types = new Class [] {
                java.lang.Object.class, java.lang.Object.class, java.lang.Object.class, java.lang.Object.class, java.lang.Double.class, java.lang.Object.class, java.lang.Object.class, java.lang.Double.class
            };
        ...
        tblHeader.setFont(new java.awt.Font("Tahoma", 0, 12));
        tblHeader.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {

            },
            new String [] {
                "PO No.", "Tanggal", "SupplierID", "Nama Supplier", "Remark"
            }
        ) {
            boolean[] canEdit = new boolean [] {
        ...
        jLabel3.setText("Koreksi PO");

        jLabel4.setForeground(new java.awt.Color(0, 0, 102));
        jLabel4.setText("Print PO");
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                .addContainerGap()
                .addComponent(jXTitledPanel1, javax.swing.GroupLayout.DEFAULT_SIZE, 814, Short.MAX_VALUE)
                .addContainerGap())
            .addGroup(layout.createSequentialGroup()
                .addContainerGap()
...
//    }
    ...
    private void previewPO(String sNo_PO){
      try{
            ...
            System.out.println("Nomor PO "+sNo_PO);
            ...
            switch(keyKode){
               case KeyEvent.VK_ENTER : {
                ...
                case KeyEvent.VK_F12:{
                    previewPO(tblHeader.getValueAt(tblHeader.getSelectedRow(), 0).toString());
                    break;
                }
