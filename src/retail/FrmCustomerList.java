 ...
 * @author  oestadho
 */
public class FrmCustomerList extends javax.swing.JInternalFrame {
...
//        masterTable.getColumnModel().removeColumn(masterTable.getColumn("Resort"));
        ...
        masterTable.getColumn("Nama Customer").setPreferredWidth(150);
    ...
    private void udfFilter(){
        ...
        String s="select kode_cust, coalesce(nama,'') as nama, coalesce(alamat_1, '') as alamat, coalesce(telepon,'') as telepon, coalesce(kontak,'') as kontak " +
                "from r_customer " +
                "where kode_cust<>'0000' and coalesce(nama,'') iLike '%"+txtNama.getText()+"%' and coalesce(alamat_1,'') iLike '%"+txtAlamat.getText()+"%' " +
...
//    }
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Daftar Customer"); // NOI18N
        setName("Form"); // NOI18N
        ...
        masterTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "No. Customer", "Nama Customer", "Alamat", "Telepon", "Kontak"
            }
        ) {
...
private void hiperLinkDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_hiperLinkDelActionPerformed
    ...
    if(iRow>=0){
        if(JOptionPane.showConfirmDialog(this, "Anda yakin untuk menghapus data tersebut?", "Confirm", JOptionPane.YES_NO_OPTION)==
                ...
                try {
                    int iDel = conn.createStatement().executeUpdate("Delete from r_customer where kode_cust='" + masterTable.getValueAt(iRow, 0).toString() + "'");
                
                    if(iDel>0){
                        JOptionPane.showMessageDialog(this, "Hapus data sukses!!!");
                        ((DefaultTableModel)masterTable.getModel()).removeRow(iRow);
                        
                        if(iRow>=masterTable.getRowCount() && masterTable.getRowCount()>0){
...
private void masterTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_masterTableMouseClicked
    ...
    private void udfNew() {
        FrmCustomerMaster fMaster=new FrmCustomerMaster();
        fMaster.setTitle("Customer baru");
        fMaster.setConn(conn);
        fMaster.setIsNew(true);
        fMaster.setSrcTable(masterTable);
        fMaster.setSrcModel(((DefaultTableModel)masterTable.getModel()));
    ...
    private void udfUpdate() {
        ...
        if(iRow>=0){
            FrmCustomerMaster fMaster=new FrmCustomerMaster();
            fMaster.setTitle("Update Customer");
            //fMaster.settDesktopPane(jDesktop);
            fMaster.setConn(conn);
            fMaster.setSrcModel(((DefaultTableModel)masterTable.getModel()));
            fMaster.setSrcTable(masterTable);
            fMaster.setIsNew(false);
            fMaster.setKodeCust(masterTable.getValueAt(iRow, 0).toString());
            fMaster.setBounds(0, 0, fMaster.getWidth(), fMaster.getHeight());
            jDesktop.add(fMaster, javax.swing.JLayeredPane.DEFAULT_LAYER);
            fMaster.setVisible(true);
            try{
