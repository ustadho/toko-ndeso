...
import retail.sales.FrmTrxPenjualan;

/**
 *
 * @author ustadho
 */
public class FrmCustomerMaster extends javax.swing.JInternalFrame {
        ...
        tblKontak.getModel().addTableModelListener(new TableModelListener() {
            ...
            public void tableChanged(TableModelEvent e) {
                ...
                if(!isNew && e.getType()==TableModelEvent.UPDATE){
                    ...
                    try{
                        String sQry="select fn_r_update_customer_kontak(" +
                                "'"+sOldKontak+"', " +
    ...
    public void setObjForm(Object objForm){
    ...
    private void udfLoadCustomer(){
        String sQry="select kode_cust, coalesce(nama,'') as nama_cust, coalesce(alamat_1, '') as alamat, " +
                "coalesce(c.kota,'') as kode_kota, coalesce(k.nama_kota,'') as nama_kota, " +
                "coalesce(telepon,'') as telepon, coalesce(c.fax,'') as fax, coalesce(kontak,'') as kontak , " +
                "coalesce(email,'') as email, coalesce(web,'') as web, coalesce(active, true) as active," +
                "coalesce(ket_default, '') as ket_default, coalesce(termin,'') as termin, " +
                "coalesce(catatan,'') as catatan,  coalesce(c.npwp,'') as npwp, coalesce(c.no_pkp,'') as no_pkp," +
                "coalesce(c.pajak_1,0) as pajak_1, coalesce(c.pajak_2,0) as pajak_2, coalesce(c.tipe_pajak,0) as tipe_pajak " +
                "from r_customer c  " +
                "left join m_kota k on k.kode_kota=c.kota " +
                "where kode_cust='"+sKode+"'";
        try{
            ResultSet rs=conn.createStatement().executeQuery(sQry);
            ...
            if(rs.next()){
                ...
                rs=conn.createStatement().executeQuery("select nama, coalesce(title,'') as jabatan, coalesce(telpon,'') as telepon, " +
                        "coalesce(hp,'') as hp, coalesce(email,'') as email from r_cust_kontak " +
                        "where kode_cust='"+txtKode.getText()+"' " +
                        "order by nama");
                while(rs.next()){
    ...
    private void btnDeleteKontakActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteKontakActionPerformed
        try {
            ...
            int i = conn.createStatement().executeUpdate("delete from r_cust_kontak where kode_cust='" + txtKode.getText() + "' and " +
    ...
    private void btnSaldoAwalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaldoAwalActionPerformed
        DlgSaldoAwCustomer d1=new DlgSaldoAwCustomer(JOptionPane.getFrameForComponent(this), true);
        d1.setConn(conn);
        d1.setKodeSupplier(txtKode.getText());
        d1.setSrcForm(this);
        d1.setTitle("Saldo awal customer : "+txtNama.getText()+" ("+txtKode.getText()+")");
        d1.setVisible(true);
    ...
    private void txtKotaKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtKotaKeyReleased
        fn.lookup(evt, new Object[]{lblKota},
                ...
                txtKota.getWidth()+lblKota.getWidth()+30, 120);
    ...
    private void udfInitForm() {
        ...
        try{
            ...
            cmbTermin.setSelectedItem("C.O.D");
            rs.close();
        }catch(SQLException se){
            JOptionPane.showMessageDialog(this, se.getMessage());
        }
...
//                    txtKode.requestFocusInWindow();
    ...
    private void udfSave() {
        try {
            ...
            ResultSet rs=conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE).executeQuery(
                    "select * from r_customer where kode_cust='" + sKode + "'");

            if(!rs.next()) {
                isNew=true; rs.moveToInsertRow();
                ResultSet r=conn.createStatement().executeQuery("select fn_r_get_kode_customer()");
                if(r.next())
            ...
            if(isNew) {
                ...
                if(srcModel!=null){
                    ...
                    srcTable.changeSelection(srcTable.getRowCount(), 0, false, false);
                }
                if(objForm!=null && objForm instanceof FrmTrxPenjualan){
                   this.dispose();
                   ((FrmTrxPenjualan)objForm).requestFocus();
                   ((FrmTrxPenjualan)objForm).requestFocusInWindow();
                   ((FrmTrxPenjualan)objForm).udfSetNewCustomer(txtKode.getText(), txtNama.getText());
                ...
                srcModel.setValueAt(txtKode.getText(), iRow, srcTable.getColumn("No. Customer").getModelIndex());
                srcModel.setValueAt(txtNama.getText(), iRow, srcTable.getColumn("Nama Customer").getModelIndex());
